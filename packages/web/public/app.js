...
function _formatAiText(text) {
  ...
  const flushList = () => {
    if (currentList.length > 0) {
      html +=
        '<ul class="ai-list">' +
        currentList.map((li) => `<li>${_inlineFmt(li)}</li>`).join("") +
        "</ul>";
      currentList = [];
    }
  };
...
async function fetchBranches() {
  try {
    ...
    if (branches) {
      DOM.branchSelect.innerHTML = branches
        .map(
          (b) =>
            '<option value="' +
            b +
            '" ' +
            (b === state.branch ? "selected" : "") +
            ">" +
            b +
            "</option>",
        )
...
async function refreshData() {
  ...
  try {
    const patterns = state.excludePatterns
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    const payload = {
...
function renderDashboard(data) {
  ...
  const avgHealth = Math.round(
    (health.stability + health.velocity + health.simplicity + health.coverage + health.decoupling) /
      5,
  );
  ...
  DOM.metricChurn.innerText =
    totalLines >= 1000 ? (totalLines / 1000).toFixed(1) + "k" : totalLines;
  const avgRisk = Math.round(
    riskScores.reduce((a, r) => a + r.score, 0) / (riskScores.length || 1),
  );
  DOM.metricRisk.innerText = avgRisk;
  DOM.metricRisk.className =
    "metric-value " + (avgRisk > 60 ? "risk-high" : avgRisk > 30 ? "risk-medium" : "risk-low");
  if (data.aiSummary) {
    ...
    DOM.aiSummary.innerHTML =
      "<p>AI Insights are currently disabled. Configure AI in Settings to see behavioral patterns.</p>";
...
function renderRiskList(riskScores) {
  ...
  DOM.riskList.innerHTML = riskScores
    .slice(0, 5)
    .map((r) => {
      ...
      const impactLabel = hotspot
        ? `<div class="metric-sub">${hotspot.linesImpacted.toLocaleString()} lines churned</div>`
        : "";
...
function renderChurnChart(d) {
  ...
  ag.addColorStop(0, "rgba(79,70,229,0.18)");
  ag.addColorStop(1, "rgba(79,70,229,0)");
  const dg = ctx.createLinearGradient(0, 0, 0, 260);
  dg.addColorStop(0, "rgba(239,68,68,0.12)");
  ...
  churnChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: "Deleted",
          data: d.map((x) => x.linesDeleted || x.linesRemoved || 0),
          borderColor: "#ef4444",
          backgroundColor: dg,
          fill: true,
          tension: 0.4,
...
function renderHealthRadar(h) {
    ...
    options: {
      ...
      scales: {
        r: {
          ...
          pointLabels: { color: "#0f172a", font: { size: 11, weight: "700" }, padding: 12 },
  ...
  DOM.healthLegend.innerHTML = Object.entries(h)
    .map(
      ([k, v]) =>
        '<div class="health-legend-item"><p class="health-legend-key">' +
        k +
        '</p><p class="health-legend-val">' +
        v +
        "%</p></div>",
    )
    .join("");
  ...
  timelineChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { backgroundColor: "#fff", titleColor: "#0f172a", bodyColor: "#64748b" },
      },
      scales: {
        x: {
          stacked: true,
...
function renderCompass(compass) {
  ...
  if (DOM.compassDoc) {
    DOM.compassDoc.innerHTML = _inlineFmt(
      compass.documentation || "Explore the codebase starting with essential files below.",
    );
  }
  ...
  DOM.compassList.innerHTML = compass.essentials
    .slice(0, 9)
    .map((e, i) => {
...
function renderCompassHeatmap(hotspots, riskScores) {
  ...
  if (state.data && state.data.churn) {
    state.data.churn.forEach((c) => {
      ...
      if (key)
        dayMap[key] =
          (dayMap[key] || 0) + (c.linesAdded || 0) + (c.linesDeleted || c.linesRemoved || 0);
    });
  }
  // Fallback: count file changes
  if (Object.keys(dayMap).length === 0 && state.data && state.data.meta) {
    hotspots.forEach((f) => {
      dayMap["total"] = (dayMap["total"] || 0) + (f.changeCount || 0);
    });
  }
...
function renderBurnout(burnout) {
  if (!burnout) return;
  const flags = (burnout.flags || [])
    .map(
      (f) =>
        '<div class="burnout-flag"><div class="burnout-flag-dot"></div><p class="burnout-flag-text">' +
        f +
        "</p></div>",
    )
    .join("");
  DOM.burnoutMetrics.innerHTML =
    '<div class="burnout-counters">' +
    '<div class="burnout-counter"><div class="burnout-counter-value">' +
    (burnout.afterHoursCommits || 0) +
    '</div><div class="burnout-counter-label">After-hours</div></div>' +
    '<div class="burnout-counter"><div class="burnout-counter-value">' +
    (burnout.weekendCommits || 0) +
    '</div><div class="burnout-counter-label">Weekend</div></div>' +
    "</div>" +
    flags;
  if (!burnout.contributors || burnout.contributors.length === 0) {
    DOM.burnoutList.innerHTML = '<p class="empty-state">No burnout signals detected.</p>';
    return;
  }
  DOM.burnoutList.innerHTML = burnout.contributors
    .map(
      (c) =>
        '<div class="burnout-contributor"><div><div class="burnout-contributor-name">' +
        c.author +
        "</div>" +
        '<div class="burnout-contributor-meta">' +
        c.afterHoursPercent.toFixed(0) +
        "% after-hrs - " +
        c.weekendPercent.toFixed(0) +
        "% wknd</div></div>" +
...
function renderKnowledgeSilos(knowledge) {
  if (!knowledge || knowledge.length === 0) {
    DOM.knowledgeList.innerHTML = '<p class="empty-state">No knowledge silo data.</p>';
    return;
  }
  DOM.knowledgeList.innerHTML = knowledge
    .slice(0, 8)
    .map((s) => {
      ...
      return (
        '<div class="analysis-item"><div class="analysis-item-header"><p class="analysis-filename">' +
        _fileLink(s.path, fn) +
        '</p><div class="status-dot ' +
        s.riskLevel +
        '"></div></div>' +
        ...
        bw +
        '%"></div></div>' +
        '<p class="analysis-meta" style="margin-top:0.3rem">' +
        s.authorshipPercent.toFixed(0) +
        "% ownership</p></div>"
      );
    })
    .join("");
...
function renderCoupling(coupling) {
  if (!coupling || coupling.length === 0) {
    DOM.couplingList.innerHTML = '<p class="empty-state">No coupling data.</p>';
    return;
  }
  DOM.couplingList.innerHTML = coupling
    .slice(0, 8)
    .map((l) => {
      const hf = l.head.split(/[/\\]/).pop(),
        tf = l.tail.split(/[/\\]/).pop();
      const st = Math.round(l.coupling * 100);
      const fc = st > 70 ? "danger" : st > 40 ? "warning" : "";
      return (
        '<div class="analysis-item"><div class="coupling-pair">' +
        '<span class="analysis-filename" style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' +
        _fileLink(l.head, hf) +
        "</span>" +
        ...
        _fileLink(l.tail, tf) +
        "</span></div>" +
        '<div class="coupling-strength"><div class="progress-bar-track" style="flex:1;margin-top:0"><div class="progress-bar-fill ' +
        fc +
        '" style="width:' +
        st +
        '%"></div></div><span class="coupling-pct">' +
        st +
        "%</span></div>" +
        '<p class="coupling-shared">' +
        l.sharedCommits +
        " shared commits</p></div>"
      );
    })
    .join("");
}
...
function renderImpact(impact) {
  if (!impact || impact.length === 0) {
    DOM.impactList.innerHTML = '<p class="empty-state">No impact data.</p>';
    return;
  }
  const mx = Math.max(...impact.map((i) => i.blastRadius || 0), 1);
  DOM.impactList.innerHTML = impact
    .slice(0, 6)
    .map((f) => {
      ...
      return (
        '<div class="analysis-item"><div class="analysis-item-header"><p class="analysis-filename">' +
        _fileLink(f.path, fn) +
        '</p><span style="font-size:0.65rem;font-weight:900;color:var(--primary);flex-shrink:0">~' +
        r.toFixed(1) +
        "</span></div>" +
        '<div class="progress-bar-track"><div class="progress-bar-fill" style="width:' +
        bw +
        '%"></div></div>' +
        '<p class="analysis-meta" style="margin-top:0.3rem">avg ' +
        r.toFixed(1) +
        " - max " +
        (f.maxBlastRadius || Math.ceil(r)) +
        "</p></div>"
      );
    })
    .join("");
...
function renderRot(rot) {
  if (!rot || rot.length === 0) {
    DOM.rotList.innerHTML = '<p class="empty-state">No stale files.</p>';
    return;
  }
  DOM.rotList.innerHTML = rot
    .slice(0, 8)
    .map((fp) => {
      const fn = fp.split(/[/\\]/).pop();
      return (
        '<div class="rot-item"><div class="rot-dot"></div><p class="rot-file">' +
        _fileLink(fp, fn) +
        "</p></div>"
      );
    })
...
function renderContributorsTable(contributors) {
  if (!contributors || contributors.length === 0) {
    DOM.contributorsBody.innerHTML =
      '<tr><td colspan="6" style="text-align:center;padding:1.5rem;color:var(--muted-fg);font-size:0.75rem">No contributors found.</td></tr>';
    return;
  }
  const sorted = [...contributors].sort((a, b) => b.commitCount - a.commitCount);
  DOM.contributorsBody.innerHTML = sorted
    .map((c, i) => {
      ...
      const ini = c.author
        ...
        .toUpperCase();
      return (
        '<tr><td><div class="contrib-cell">' +
        '<div class="contrib-avatar" style="background:' +
        col +
        '">' +
        ini +
        "</div>" +
        '<div><div class="contrib-name">' +
        c.author +
        '</div><div class="contrib-email">' +
        c.email +
        "</div></div>" +
        '</div></td><td class="num">' +
        c.commitCount +
        ...
        c.filesChanged +
        "</td>" +
        '<td class="num" style="color:var(--muted-fg)">' +
        c.activeDays +
        "</td></tr>"
      );
    })
